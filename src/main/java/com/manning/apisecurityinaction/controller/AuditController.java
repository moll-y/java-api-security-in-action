...
public class AuditController {
    ...
    database.withVoidTransaction(
        tx -> {
          ...
          database.updateUnique(
              "INSERT INTO audit_log(audit_id, method, path, user_id, audit_time) VALUES(?, ?, ? ,"
                  + " ?, current_timestamp)",
              auditId,
  ...
  public void auditRequestEnd(Request request, Response response) {
    database.updateUnique(
        "INSERT INTO audit_log(audit_id, method, path, status, user_id, audit_time) VALUES(?, ?, ?,"
            + " ?, ?, current_timestamp)",
  ...
  public JSONArray readAuditLog(Request request, Response response) {
    // read logs entries for the last hour.
    var since = Instant.now().minus(1, ChronoUnit.HOURS);
    var logs =
        database.findAll(
            AuditController::recordToJson,
